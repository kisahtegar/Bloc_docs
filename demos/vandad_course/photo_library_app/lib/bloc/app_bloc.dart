...
class AppBloc extends Bloc<AppEvent, AppState> {
  ...
  AppBloc() : super(const AppStateLoggedOut(isLoading: false)) {
    // Handle going to registration screen.
    ...
    on<AppEventDeleteAccount>((event, emit) async {
      ...
      try {
        ...
        // If we get AuthException while we're to delete account we going to not
        // loading, we get user, grabing images that were in the current states
        // and we wraping the error inside an author and putting it inside
        // AppStateLoggedIn.
        emit(
          AppStateLoggedIn(
            isLoading: false,
    ...
    on<AppEventUploadImage>((event, emit) async {
      final user = state.user; // Grab user from our AppState.
  ...
  /// This function used to retrieve images for userId.
